 ...
 * precedente que NotesEngineScreen.tsx) — toda la UI/lógica de
 * FinanceScreen.tsx original vive acá, parametrizada por `engine` (el
 * `useEngine()` de createFinanceEngine.ts). La captura automática
 * Umbral→Finanzas es EXCLUSIVA del Finanzas general del usuario: "Mi
 * proyecto" no tiene Umbral propio, así que `ideaCapture` es opcional —
 * cuando se omite, el efecto de auto-conversión nunca corre, "Sin
 * monto" nunca aparece y borrar un movimiento nunca toca ninguna Idea
 * (ver `FinanceScreen.tsx`, que arma este prop, y `MiProyectoScreen.tsx`,
 * que lo omite).
 */
export interface FinanceEngineIdeaCapture {
  /** Ideas con `destino === 'finanzas'`, sin filtrar por ya-convertidas — ese filtro (`convertidas`/`enConversion`) vive acá adentro, contra `movimientos` del motor. */
  ideas: readonly { id: string; texto: string; fecha: string }[]
  /** Saca la Idea de 'finanzas' sin borrar su texto — mismo `moveSheet(idea, 'archivador')` para "Sin monto" y para el cleanup de `eliminarMovimiento`. */
  onDescartar(ideaId: string): void
}
 ...
 * el Umbral — mismo `engine.addMovimiento` que ya usa el efecto de
 * abajo, ninguna escritura paralela. Lo único que sigue esperando en
 * "Sin monto" es lo que no trae número: ahí no hay nada que registrar
 * todavía, solo pedir que se reescriba con la cifra.
 *
...
export function FinanceEngineScreen({ engine, ideaCapture }: FinanceEngineScreenProps) {
  ...
  /** Capturas que el Umbral mandó acá y todavía no encontraron un monto. Vacío siempre que no hay `ideaCapture` (Mi Proyecto no tiene Umbral propio). */
  const pendientes = (ideaCapture?.ideas ?? []).filter(
    (idea) => !convertidas.has(idea.id) && !enConversion.current.has(idea.id),
  )
  ...
  useEffect(() => {
    ...
    // Se re-ejecuta cuando cambian ideaCapture.ideas o movimientos: cada
    // alta reduce `pendientes` en el próximo render, hasta que solo
    // quedan las que de verdad no traen un monto (esas no vuelven a
    // intentarse). Con `ideaCapture` ausente, `pendientes` es siempre
    // `[]` y este efecto nunca escribe nada.
   ...
   * al toque y, peor, otra vez después de recargar. Mismo `onDescartar`
   * que usa "Descartar" en "Sin monto" más abajo: saca la Idea de
   * 'finanzas' sin borrar el texto que el usuario escribió. No hace
   * nada cuando no hay `ideaCapture` (Mi Proyecto no genera `ideaId`).
   */
  function eliminarMovimiento(movimiento: FinanceMovimiento) {
    void deleteMovimiento(movimiento.id)
    if (movimiento.ideaId && ideaCapture) {
      ideaCapture.onDescartar(movimiento.ideaId)
    }
  }
              ...
              <li key={idea.id} className="border-b border-border/40 py-3 last:border-b-0">
                ...
                <p className="mt-1 flex items-baseline gap-2 text-[13px] text-ink-faint">
                  ...
                  <button
                    ...
                    onClick={() => ideaCapture?.onDescartar(idea.id)}
                  >
                    ×
                  </button>
                </p>
