...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      severity: 'error',
      comment: 'un módulo no puede depender del árbol de aplicación todavía no migrado (ARCHITECTURE_RATIFIED.md §7). Excluye a `today`, que tiene su propia regla dedicada (today-app-tree-boundaries) por ser el orquestador declarado. Excepciones universales documentadas: (1) src/lib/db/db.ts es la instancia Dexie compartida que todo Repository necesita — todavía no migrada a shared-kernel/persistence. (2) src/app/shell/comprehensionEngine.ts es el singleton de composición que work-table/IdeaCapture.tsx consume directamente — Foundation §17 fase 3 lo corrige formalmente. (3) src/components/ui/EmptyState.tsx es UI compartida (usada por missions, habits, journal, trading, library) todavía no migrada a estudio-design-system (F9 solo migró tokens, sin componentes). (4) src/lib/reminders/ (Fase 3, push real) es el puente compartido entre agenda y missions hacia la tabla `recordatorios` — no puede vivir en shared-kernel porque depende de src/lib/db/db.ts y src/types/recordatorio.ts, ambos fuera de lo que shared-kernel-is-a-leaf permite importar. (5) src/components/notes-engine/ es el motor de carpetas+notas compartido entre `notes` y `miproyecto` (mismo precedente que EmptyState: UI/lógica compartida entre módulos, todavía no migrada a shared-kernel/estudio-design-system). (6) src/components/finance-engine/ es el motor de Finanzas (cuentas, movimientos, metas, períodos de cobro) compartido entre `finance` y `miproyecto` — mismo precedente que (5): UI/lógica compartida entre módulos, todavía no migrada a shared-kernel/estudio-design-system. Condición de remoción: (1) cuando db.ts se relocalice a shared-kernel/persistence; (2) cuando se ejecute esa fase 3; (3) cuando EmptyState se relocalice a estudio-design-system; (4) cuando exista una relocalización formal de recordatorios a shared-kernel/persistence (post migración de (1)); (5) cuando notes-engine se relocalice a shared-kernel/estudio-design-system; (6) cuando finance-engine se relocalice a shared-kernel/estudio-design-system.',
      from: { path: '^src/modules', pathNot: '^src/modules/today/' },
      to: {
        path: '^src/(app|features|components|lib|types|main\\.tsx|App\\.tsx)',
        pathNot: '^src/lib/db/db\\.ts$|^src/app/shell/comprehensionEngine\\.ts$|^src/components/ui/EmptyState\\.tsx$|^src/lib/reminders/|^src/components/notes-engine/|^src/components/finance-engine/',
      },
    },
    {
